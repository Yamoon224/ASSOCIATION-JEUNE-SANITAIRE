<x-app-layout>
        ...
        <div class="container d-block mt-2">
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title text-center">@lang('locale.nfs')</h4>
                            
                            <form action="{{ route('nfs.update', $nfs->id) }}" method="post">
                                @csrf
                                @method('PUT')
                                <div class="row overflow-auto">
                                    ...
                                    <div class="col-md-4 col-sm-6 col-xs-12 mb-2">
                                        <select name="patient_id" id="patient_id" class="form-control" required>
                                            ...
                                            <option value="{{ $item->id }}" title="{{ $item->gender."_".$item->age }}" {{ $nfs->patient_id == $item->id ? 'selected' : '' }}>{{ $item->firstname." ".$item->name." | ".$item->age." ".__('locale.old_years')." | ".$item->gender }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4 col-sm-6 col-xs-12 mb-2">
                                        <select name="doctor_id" id="doctor_id" class="form-control" required>
                                            ...
                                            <option value="{{ $item->id }}" {{ $nfs->doctor_id == $item->id ? 'selected' : '' }}>{{ $item->firstname." ".$item->name." | ".$item->phone." | ".$item->speciality->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-2"></div>
    ...
    <input type="hidden" name="id" value="{{ $nfs->id }}">
    @push('scripts')
        ...
        <script>
            ...
            $('#form-parameters').load("{{ route('nfs.add') }}", {
